...
// Compatible with map4 dynamic loading system

import olVectorLayer from 'https://cdn.skypack.dev/ol/layer/Vector.js';
import olVectorImage from 'https://cdn.skypack.dev/ol/layer/VectorImage.js';
import olVectorSource from 'https://cdn.skypack.dev/ol/source/Vector.js';
import olStyle from 'https://cdn.skypack.dev/ol/style/Style.js';
import olCircle from 'https://cdn.skypack.dev/ol/style/Circle.js';
import olFill from 'https://cdn.skypack.dev/ol/style/Fill.js';
import olStroke from 'https://cdn.skypack.dev/ol/style/Stroke.js';
import olText from 'https://cdn.skypack.dev/ol/style/Text.js';
import olGeoJSON from 'https://cdn.skypack.dev/ol/format/GeoJSON.js';
import { transformExtent as olTransformExtent } from 'https://cdn.skypack.dev/ol/proj';
import { bbox as olBbox } from 'https://cdn.skypack.dev/ol/loadingstrategy.js';
...
const ly_sxid_geo_nacrt_style = function (feature) {
  ...
  if (geomType === "Point") {
    ...
    let ly_sxid_geo_nacrt = new olVectorLayer({
  ...
  let ly_cloudfiles = new olVectorLayer({
      id: "lyid_cloudfiles",
      name: "Slike",
      source: new olVectorSource({
        url: function (extent) {
          let ext2 = olTransformExtent(extent, appContext.mapproj, d96proj);
          //let u="_sx1/sxtables/sxid_cloudfiles/data/.json?select=geometry,gsx_id,fname,dname,date_modified,owner_firstname&bbox=" + ext2.join(",");          
...
const ly_mg_parcele_style_func = function (feature, resolution) {
  ...
  return ly_mg_parcele_style_cache.get(cacheKey);
    ...
    let ly_mg_parcele = new olVectorImage({
      id: "lyid_mg_parcele",
      name: "Moje parcele",
      //declutter: true,
      ...
      source: new olVectorSource({
        url: function (extent) {
          let ext2 = olTransformExtent(extent, appContext.mapproj, d96proj);
          return (
            "_sx1/sxtables/sxid_proj_pogoji_parcele_view/data/.json?select=gsx_id,geometry,eid_parcela,ko_id,st_parcele,jd,imeko,last1_ime,povrsina,namenska_raba,upravni_status,sluznost,soglasje,mnenja,pridob_vse,datum_sluznost_soglasje_gradnja,datum_mnenja,sluznost_dn,soglasje_dn,mnenja_dn&bbox=" + ext2.join(","));
        },
        format: new olGeoJSON({
          dataProjection: d96proj,
          featureProjection: appContext.mapproj,
...
function describeFeature(feature, layer) {
  ...
  return {
    table: [
      ['Layer', layerId],
      ['Feature ID', feature.getId() || 'N/A']
    ]
  };
}

// Exports for map4 compatibility
export { 
  myLayers,
  describeFeature,
  sidebarItems,
